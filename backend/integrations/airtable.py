...
# Airtable OAuth2 constants from environment variables
...
scope = os.getenv('AIRTABLE_SCOPES', 'data.records:read data.records:write data.recordComments:read data.recordComments:write schema.bases:read schema.bases:write')
...
async def get_items_airtable(credentials) -> list[IntegrationItem]:
    ...
    for response in list_of_responses:
        ...
        if tables_response.status_code == 200:
            ...
            print(f"Error fetching tables for base {response.get('id')}: {tables_response.status_code}")

    print(f'Airtable items fetched: {len(list_of_integration_item_metadata)} (Bases and Tables)')
