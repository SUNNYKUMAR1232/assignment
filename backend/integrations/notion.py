...
# Notion OAuth2 constants from environment variables
...
async def get_items_notion(credentials) -> list[IntegrationItem]:
    ...
    if response.status_code == 200:
        ...
        print(f'Notion items fetched: {len(list_of_integration_item_metadata)}')
        return list_of_integration_item_metadata
    else:
        print(f"Error fetching Notion items: {response.status_code} - {response.text}")
        return []
